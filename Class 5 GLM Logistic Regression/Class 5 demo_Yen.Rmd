---
title: "GLM logistic demo(Yen's notes"
...
p <- ggplot(BRFSS[!is.na(BRFSS$diabetes_binary) & !is.na(BRFSS$bmi), ]) #plot and exclude NA's
  ...
  p
```
##Test assumptions of linearity, influence, and check percent correctly predicted for BMI in a logistic model by creating term to test linearity by continous variables*log(continous variable)

```{r}
#linearity
#create term to test linearity by continous variables*log(continous variable), if it is significant, it means it might violate linearity
bmi.times.logbmi<-BRFSS$bmi*log(BRFSS$bmi)
